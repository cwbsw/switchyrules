var FindProxyForURL = function(init, profiles) {
    ...
    "+gfwlist": "+__ruleListOf_gfwlist",
    "+__ruleListOf_gfwlist": function(url, host, scheme) {
        "use strict";
        ...
        if (scheme === "http" && url.indexOf(".firefoxfan.cc") >= 0) return "+proxy";
        if (/^http:\/\/firefoxfan\.cc/.test(url)) return "+proxy";
        ...
        if (/(?:^|\.)kusos\.com$/.test(host)) return "+proxy";
        ...
        if (/^http:\/\/nchc\.dl\.sourceforge\.net/.test(url)) return "+proxy";
        ...
        if (/^http:\/\/zhenlibu1984\.com/.test(url)) return "+proxy";
